...
```
1コミット = 1つのドキュメント層の変更（推奨）
```

層ごとにコミットを分けることを推奨する。順序はプロジェクトの慣習に従う。
...
フロー [A]〜[C] の各ステップでコミットを作成する際は上記規約を推奨する。
ただし、以下の場合はそちらを優先する:
- ユーザーが明示的に「まとめてコミットして」「saveして」等と指示した場合
- プロジェクトに既存のコミット規約がある場合（そちらの順序・粒度に従う）
