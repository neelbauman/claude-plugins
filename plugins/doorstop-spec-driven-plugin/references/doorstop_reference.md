...
すべてのスクリプトは `${CLAUDE_PLUGIN_ROOT}/scripts/` にある。
...
```bash
# 基本構文
uv run python ${CLAUDE_PLUGIN_ROOT}/scripts/core/doorstop_ops.py <project-dir> <command> [options]
```
...
- すべてのスクリプトは `uv run python ${CLAUDE_PLUGIN_ROOT}/scripts/<script>.py` で実行する
